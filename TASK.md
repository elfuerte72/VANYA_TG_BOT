...
- [ ] Установить библиотеку `aiogram` для работы с Telegram API.
- [ ] Установить `openai` для интеграции с OpenAI API.
- [ ] Установить необходимые дополнительные пакеты (`sqlcipher`, `pytest` и др.).
...
- [ ] Создать класс для работы с OpenAI API (ChatGPT 4.1 nano).
- [ ] Реализовать обработку ошибок при запросах к API.
- [ ] Создать механизм кэширования ответов для экономии запросов.
...
- [ ] После первого расчёта устанавливать `calculated = true` и записывать `calculated_at` (текущая дата/время).
- [ ] При последующих обращениях проверять этот флаг и отправлять сообщение: «Вы уже получили расчёт КБЖУ».
...
3.3.2. **Сбор данных**
- [ ] Хэндлеры для каждого параметра:
...
- [ ] При пропуске выбора активности автоматически использовать значение 1.55 (Средний).
...
- [ ] Вынести в `services/calculation.py` функции по формуле Бенедикта:
